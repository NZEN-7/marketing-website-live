...
- **Every submission also writes a Supabase row** (`recordLead`, project
  **CRM** `skyequfcoejlhzbyipwt`, `ap-southeast-2`). The email is still sent
  ...
  `public.leads` is **inbound only** (27 Aug 2026): as-submitted, never edited,
...
system, Brand & Style, the CRM): on 25 Sep a whole evening's work landed in the
personal Drive by mistake. Product requirements come from the PRDs in
